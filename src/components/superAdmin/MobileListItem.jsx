...
export default function MobileListItem({ data, onEdit, onDelete, contextHolder, onClick }) {
  return (
    <>
      {data && data.length > 0 ? (
        data.map((item) => {
              ...
              >
                <div className="flex w-full justify-between items-center">
                  <div className="flex gap-1 items-center">
                    <HiOutlineIdentification className="text-blue-600" />
                ...
                <div className="h-full flex flex-col justify-between" onClick={() => onClick && onClick(item)}>
                  <div className="h-14 bg-gray-3 rounded border border-gray-5 flex justify-center px-2">
                    <div className="w-6/12 h-full flex flex-col justify-center items-start">
                      <p className="text-xs text-gray-12/70">Item Name/Number</p>
                      <p className="text-sm truncate w-full">
                        {item?.displayname || '-'}
                      </p>
                    </div>
                    <div className="w-6/12 h-full flex flex-col justify-center items-start">
                      <p className="text-xs text-gray-12/70">Display Name/Code</p>
                      <p className="text-sm truncate w-full">
                        {item?.itemid || '-'}
                      </p>
                    </div>

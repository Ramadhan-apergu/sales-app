...
import { HiMiniInbox, HiOutlineIdentification, HiCheckCircle, HiXCircle, HiCalendarDateRange } from "react-icons/hi2";
...
export default function MobileListAgreement({ data, onEdit, onDelete, contextHolder, onClick }) {
  return (
    <>
      {data && data.length > 0 ? (
        data.map((item) => {
            ...
            <div key={item.id} className="pb-4">
              ...
              >
                ...
                <div className="h-full flex flex-col justify-between" onClick={() => onClick && onClick(item)}>
                  <div className="h-14 bg-gray-3 rounded border border-gray-5 flex justify-center px-2">
                    <div className="w-6/12 h-full flex flex-col justify-center items-start">
                      <p className="text-xs text-gray-12/70">Agreement Name</p>
                      <p className="text-sm truncate w-full">
                        {item?.agreementname || '-'}
                      </p>
                    </div>
                    <div className="w-6/12 h-full flex flex-col justify-center items-start">
                      <p className="text-xs text-gray-12/70">Category</p>
                      <p className="text-sm truncate w-full">
                        {item?.category || '-'}
                      </p>
                    </div>
                ...
                <div className="w-full flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1">
                        <HiCalendarDateRange className="text-blue-6" />
                        <p className={`capitalize text-xs`}>
                        {formatDateToShort(item?.effectivedate)}
                        </p>
                    </div>
                </div>
              </div>

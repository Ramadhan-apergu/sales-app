...
function UserRoles() {
  ...
  const handleEdit = (record) => {
    router.push(`/super-admin/access-control/${title}/${record.id}/edit`);
  };
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Link href={`/super-admin/access-control/${title}/${record.id}`}>
          {text || "-"}
        </Link>
      ),
    },
      ...
      <div className="w-full flex flex-col gap-4">
        <div className="w-full flex justify-between items-center">
          ...
          <Button
            ...
            icon={<PlusOutlined />}
            onClick={() =>
              router.push(`/super-admin/access-control/${title}/new`)
            }
          >
            {isLargeScreen ? `Enter` : ""}
          </Button>
